...
N = 10000;
h = [1 0.5];%[1 0.5];%[0.407 0.815 0.407];
poly_coeff = [1 0 -0.9];%[1 0 -0.9];%[1 0 0];%[1 0 -0.9];
...
parfor nsim = 1:nsims
    ...
    SVMModels = train_svm_modelBhaskar(N, h, poly_coeff,coloredNoise,signal_to_noise(nsim), 'plot');
...
%     load SVMModels
    ...
    for iter = 1:niter
        ...
        noisePower = sigPower/(10^(signal_to_noise(nsim)/10));
...
%         y = awgn(x_hat, signal_to_noise(nsim));
