...
for nsim = 1:nsims
    ...
    fprintf("Running for SNR: %d (%d/%d) ... ", signal_to_noise(nsim), nsim, nsims)
    ...
    SVMModels = train_svm_model(N, h, D, poly_coeff, signal_to_noise(nsim), 'plot', gamma, delta);
...
%     load SVMModels
    ...
    for iter = 1:niter
        ...
        Xrange = max(max(X))-min(min(X));
        X = 2*((X-min(min(X)))/Xrange)-1;
...
% BER_sig_D_0 = BER_SVM;
% save('BER_sig_D_0.mat','BER_sig_D_0')
% 
