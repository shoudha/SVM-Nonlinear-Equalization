...
parfor nsim = 1:nsims
    ...
    [u_train, X_train, SVMModels] = train_svm_model(N, h, D, poly_coeff, signal_to_noise(nsim), 'noplot');
...
%         bits_ml = H1>H0;
        ...
        %BER calculation
        SVM_errs = SVM_errs + sum(bits_tl~=bits((1+D):(D+length(bits_tl))))/niter;
%         ML_errs = ML_errs + sum(bits_ml~=bits((1+D):(D+length(bits_ml))))/niter;
...
BER_rbf_D_2 = BER_SVM;
save('BER_rbf_D_2.mat','BER_rbf_D_2')
